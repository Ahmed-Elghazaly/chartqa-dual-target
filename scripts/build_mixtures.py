...
def refchartqa_records(*, cap: int, cache: Path) -> list[ChartRecord]:
    ...
    **The enrichment must be attached here, not merged later.** A mixture stores record ids
    ...
    # No plans are attached here. Ahmed: all mining is the LLM's, including the 833 that
    # came from RefChartQA's own gold derivations (`DECISIONS.md` 0141). Alignment gives a
    # box its label and value; that is not mining and it stays.
    return records[:cap]
