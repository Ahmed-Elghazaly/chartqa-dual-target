...
def push_dataset(api, staging: Path, *, notes: str) -> str:
    ...
    if exists:
        api.dataset_create_version(str(staging), version_notes=notes, dir_mode="zip", quiet=False)
    else:
        api.dataset_create_new(str(staging), public=False, dir_mode="zip", quiet=False)
    return full


KERNEL_SCRIPT = '''"""Generated by scripts/kaggle_run.py. Do not edit here."""
...
print("=" * 70, flush=True)
subprocess.run(["find", WORK, "-maxdepth", "2", "-name", "*.toml", "-o", "-maxdepth", "2", "-type", "d"], check=False)

root = WORK
if not os.path.exists(os.path.join(root, "pyproject.toml")):
    for cand in glob.glob(os.path.join(WORK, "*", "pyproject.toml")):
...
# Surface results in the kernel output.
for pattern in ("/kaggle/working/cdt-outputs/**/smoke_results.json",
    ...
    for f in glob.glob(pattern, recursive=True):
        dst = os.path.join("/kaggle/working", os.path.basename(os.path.dirname(f)) + "__" + os.path.basename(f))
        shutil.copy2(f, dst)
...
def push_kernel(api, dataset_slug: str, command: list[str], *, title: str, gpu: bool = True) -> str:
    ...
    (staging / "main.py").write_text(
        KERNEL_SCRIPT.format(dataset=DATASET_SLUG, command=json.dumps(command)),
        encoding="utf-8",
    )
