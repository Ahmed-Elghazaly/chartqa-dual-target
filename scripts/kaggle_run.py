...
def _fingerprint(staging: Path) -> str:
    """SHA-256 over every staged .py/.yaml file, path-sensitive and order-stable.

    Kaggle attaches the latest *ready* dataset version at kernel start. A version
    uploaded seconds earlier may still be processing, in which case the kernel
    silently receives the PREVIOUS one -- and runs code you already fixed. That
    happened: version 10 was uploaded at 17:56:05, the kernel started at 17:56:07,
    and the run reproduced two bugs that had been fixed and pushed.

    Asking for the right code and checking you got it are separate acts.
    """
    h = hashlib.sha256()
    for path in sorted(staging.rglob("*")):
        if path.is_file() and path.suffix in (".py", ".yaml", ".toml"):
            h.update(path.relative_to(staging).as_posix().encode())
...
def push_dataset(api, staging: Path, *, notes: str) -> str:
    ...
    before = _dataset_version(api, full) if exists else 0
    if exists:
    ...
    # A version that is still processing is not the version the kernel will get.
    deadline = time.time() + 300
    while time.time() < deadline:
        status = str(_dataset_status(api, full)).lower()
        version = _dataset_version(api, full)
        if "ready" in status and version > before:
            print(f"  dataset version {version} ready")
            return full
        time.sleep(10)
    print(f"  WARNING: dataset did not report a new ready version within 300 s "
          f"(status={_dataset_status(api, full)}); the kernel's fingerprint check will catch staleness")
    return full


def _dataset_status(api, full: str) -> str:
    try:
        return str(api.dataset_status(full))
    except Exception:  # noqa: BLE001
        return "unknown"


def _dataset_version(api, full: str) -> int:
    try:
        for d in api.dataset_list(mine=True):
            r = d if isinstance(d, dict) else json.loads(str(d))
            if str(r.get("ref", "")).endswith(full.split("/")[-1]):
                return int(r.get("currentVersionNumber") or 0)
    except Exception:  # noqa: BLE001
        pass
    return 0
...
# Verify we received the code that was uploaded, not a previous dataset version.
_fp_path = os.path.join(root, "CODE_FINGERPRINT.txt")
_fp = open(_fp_path).read().strip() if os.path.isfile(_fp_path) else "(absent)"
print("code fingerprint: got", _fp, "| expected", EXPECTED_FINGERPRINT, flush=True)
if _fp != EXPECTED_FINGERPRINT:
    raise SystemExit(
        "STALE CODE: this kernel received dataset content with fingerprint " + _fp +
        " but the run was launched with " + EXPECTED_FINGERPRINT + ". Kaggle attached an "
        "older dataset version, most likely because the new one was still processing. "
        "Re-run; the uploader now waits for readiness."
    )
...
def render_kernel_script(
    dataset: str, command: list[str], *, allow_cpu: bool = False, fingerprint: str = "unset"
) -> str:
    """Render the kernel script, and refuse to ship one that does not parse."""
    script = KERNEL_SCRIPT.format(
        dataset=dataset,
        command=json.dumps(command),
        allow_cpu=repr(bool(allow_cpu)),
...
def push_kernel(
    ...
    machine_shape: str = MACHINE_T4, fingerprint: str = "unset",
) -> str:
    user = _username()
    ...
    (staging / "main.py").write_text(
        render_kernel_script(DATASET_SLUG, command, allow_cpu=not gpu, fingerprint=fingerprint),
        encoding="utf-8",
    )
...
def main() -> int:
    ...
    sha = _git_sha()
    fingerprint = _fingerprint(staging)
    (staging / "CODE_FINGERPRINT.txt").write_text(fingerprint, encoding="utf-8")
    print(f"staged {n} files from {REPO_ROOT.name} @ {sha}  fingerprint={fingerprint}")
